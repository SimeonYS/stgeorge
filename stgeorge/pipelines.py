...
class SstgeorgePipeline:

    # Database setup
    conn = sqlite3.connect('stgeorge.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `stgeorge`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM stgeorge WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO stgeorge (date, title, link, content)"
